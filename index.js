...
async function run() {
  try {
    ...
    app.get('/service/:id', async (req, res) => {
        const id = req.params.id;
        const options = {
            projection: { title: 1, price: 1, service_id:1, img:1, title:1 },
        }
        const result = await servicesCollection.findOne({_id: new ObjectId(id)}, options)
        res.send(result)
    })
...
app.get("/", (req, res) => {
    res.send("cars doctor server is running perfectly...")
})
